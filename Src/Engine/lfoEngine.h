  ...
  float next() {
    ...
    switch (lfo_->type()) {
      case Lfo::LINEAR:
        ...
        break;
      case Lfo::EXP_LOG:
        x = LookupTablesUtils::readInterpolated((stage_ == RISING) ? lut_exp : lut_inv_exp, skewPhase);
        break;
      case Lfo::LOG_EXP:
        x = LookupTablesUtils::readInterpolated((stage_ == RISING) ? lut_inv_exp : lut_exp, skewPhase);
        break;
      case Lfo::SQUARE:

...
class EnvelopeList : public SettingsList {
 public:
  ...
  const char* valueText(int item) override {
    ...
    switch (item) {
      case ENVELOPE:      return SettingsText::intToText(settings_->envelopeIndex() + 1);
      case MODE:          return envelope.modeText();
      case CLOCK_SYNC:    return envelope.clockSyncText();
      case ATTACK_TIME:   return envelope.attackTimeText();
      case ATTACK_SHAPE:  return envelope.attackShapeText();
      case DECAY_TIME:    return envelope.decayTimeText();
      case DECAY_SHAPE:   return envelope.decayShapeText();
      case SUSTAIN_LEVEL: return envelope.sustainLevelText();
      case HOLD_TIME:     return envelope.holdTimeText();
      case RELEASE_TIME:  return envelope.releaseTimeText();
      case RELEASE_SHAPE: return envelope.releaseShapeText();
      case LOOP:          return envelope.loopText();
      case INVERT:        return envelope.invertText();
      default:
        break;
    }
    return nullptr;

...
public:
	...
	static int8_t buttonToChapter(uint8_t id) {
		switch (id)
		{
	...
	static int8_t encoderToFunction(uint8_t id) {
		switch (id)
		{
	...
	static bool encoderIsPressed(uint8_t encoder_id) {
		switch (encoder_id)
		{
		case FUNCTION_ENC_A:	return isPressed(FUNCTION_ENC_PUSH_A);
		case FUNCTION_ENC_B:	return isPressed(FUNCTION_ENC_PUSH_B);
		case FUNCTION_ENC_C:	return isPressed(FUNCTION_ENC_PUSH_C);
		case FUNCTION_ENC_D:	return isPressed(FUNCTION_ENC_PUSH_D);
...
private:
	static inline uint32_t pressed_[(8 * 8) / 32];
};
